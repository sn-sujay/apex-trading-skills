...
def optimize_strategy_weights(metrics, state, trades):
    ...
    weights = state.get('strategy_weights', {
        'BREAKOUT': 0.30, 'MOMENTUM': 0.25, 'MEAN_REVERSION': 0.20,
        'MTF_EMA': 0.15, 'OPTION_SPREAD': 0.10
    }).copy()
    ...
    for strategy in weights:
        ...
        if len(st_trades) >= 5:
            ...
            if wr > 0.55:
                old = weights[strategy]
                weights[strategy] = min(weights[strategy] * 1.1, 0.40)
                if weights[strategy] != old:
                    changes.append(f"{strategy}: {old:.2f} -> {weights[strategy]:.2f}")
            elif wr < 0.40:
                old = weights[strategy]
                weights[strategy] = max(weights[strategy] * 0.9, 0.05)
                if weights[strategy] != old:
                    changes.append(f"{strategy}: {old:.2f} -> {weights[strategy]:.2f}")
    ...
    for s in weights:
        weights[s] = round(weights[s] / total, 2)
